...
const navigation = [
  { label: 'Route Command', icon: LayoutGrid },
  { label: 'Crew Directory', icon: Users },
  { label: 'Live Route Map', icon: Map },
  { label: 'Fleet Operations', icon: Truck },
  { label: 'Performance Trends', icon: BarChart3 }
];

export const VSRPlatformShell: React.FC<VSRPlatformShellProps> = ({ user, onSignOut, children }) => {
    ...
    <div className="flex h-full flex-col justify-between">
      <div>
        ...
        <nav className="space-y-1 p-3">
          {navigation.map(({ label, icon: Icon }, index) => <button key={label} onClick={() => setMobileOpen(false)} className={`flex w-full items-center gap-3 rounded-lg border px-3 py-2.5 text-sm font-medium transition ${index === 0 ? 'border-[#92C842]/30 bg-[#92C842]/10 text-[#92C842] shadow-[0_0_12px_rgba(146,200,66,0.08)]' : 'border-transparent text-slate-400 hover:bg-[#151f38] hover:text-slate-100'}`}><Icon size={16} /><span>{label}</span></button>)}
          <div className="mt-2 border-t border-[#1e2d4d]/60 pt-2"><button onClick={() => setPreferencesOpen((open) => !open)} className="flex w-full items-center justify-between rounded-lg border border-[#1e2d4d] bg-[#151f38] px-3 py-2 text-left text-xs font-semibold text-slate-300 hover:border-[#92C842]/40 hover:text-white"><span className="flex items-center gap-2.5"><Settings2 size={16} className="text-[#92C842]" />Telemetry Preferences</span><span className="flex items-center gap-1 font-mono text-[10px] text-[#92C842]"><span>30m</span><ChevronDown size={13} className={preferencesOpen ? 'rotate-180' : ''} /></span></button>{preferencesOpen && <div className="mt-2 rounded-xl border border-[#1e2d4d] bg-[#090e1c] p-3 text-[11px] text-slate-400"><div className="flex items-center gap-2"><Clock3 size={13} className="text-[#F17F31]" /><span>GPS freshness threshold</span><span className="ml-auto font-mono text-[#92C842]">30 min</span></div><div className="mt-3 grid grid-cols-4 gap-1">{[15, 30, 45, 60].map((minutes) => <button key={minutes} className={`rounded py-1 font-mono text-[10px] ${minutes === 30 ? 'bg-[#92C842] font-bold text-[#090e1c]' : 'border border-[#1e2d4d] bg-[#151f38] text-slate-400'}`}>{minutes}m</button>)}</div></div>}</div>
          <div className="mt-2 border-t border-[#1e2d4d]/60 pt-2"><button className="flex w-full items-center gap-3 rounded-lg border border-[#1e2d4d] bg-[#151f38] px-3 py-2 text-left text-xs font-semibold text-slate-300 hover:border-[#92C842]/40 hover:text-white"><FileText size={16} className="text-[#92C842]" /><span>Shift Compliance (PDF)</span></button></div>
        </nav>
      </div>
