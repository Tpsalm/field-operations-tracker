...
export const FieldMerchandisersView: React.FC<FieldMerchandisersViewProps> = ({
  ...
  const [hubModal, setHubModal] = useState<FieldMerchandiserHub | null>(null);
  const [hubModalSearch, setHubModalSearch] = useState<string>('');
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        {hubs.map((hub) => (
          <div
            key={hub.hub}
            onClick={() => setHubModal(hub)}
            className="p-4 rounded-[12px] border transition-all cursor-pointer shadow-[0_2px_8px_rgba(0,0,0,0.03)] bg-white border-slate-200/80 hover:border-emerald-400 hover:shadow-md group flex flex-col justify-between"
            title={`Click to view full tabular data of all staff & machines in ${hub.hubDisplayName}`}
          >
            <div>
              <div className="flex items-center justify-between">
                <span className="text-xs font-bold text-slate-800">{hub.hubDisplayName}</span>
      ...
      {/* HUB KPI TABULAR DRILL-DOWN MODAL */}
      {hubModal && (
        <div className="fixed inset-0 z-[9999] flex items-center justify-center p-3 sm:p-5 bg-slate-900/60 backdrop-blur-xs animate-in fade-in duration-200">
          <div className="bg-white rounded-[16px] border border-slate-200/90 shadow-2xl w-full max-w-5xl max-h-[85vh] flex flex-col overflow-hidden animate-in zoom-in-95 duration-200">
            {/* Modal Header */}
                ...
                <div>
                  <div className="flex items-center gap-2">
                    <span className="text-[10px] font-mono font-bold uppercase tracking-wider px-2 py-0.5 rounded bg-emerald-100 text-emerald-800">
                      Regional Branch Roster
                    </span>
                    <span className="text-xs text-slate-500 font-mono">07:00–21:00 WAT</span>
                  </div>
                  <h3 className="text-base font-black text-slate-900 mt-0.5">
                    {hubModal.hubDisplayName} — Store Workers &amp; Active POS Terminals
                  </h3>
                </div>
              </div>
              <div className="flex items-center gap-2">
                  ...
                  onClick={() => {
                    const rows = terminals
                      ...
                      .map(t => [
                        ...
                        `"${t.status}"`,
                        `"${t.battery}%"`,
                        `"${t.hourlyTransactions}"`
                      ]);
                    const csvContent = ['Terminal Code,Store Name,Zone,Staff Name,Status,Battery,Hourly Tx', ...rows.map(r => r.join(','))].join('\n');
                    const blob = new Blob([csvContent], { type: 'text/csv;charset=utf-8;' });
                    const url = URL.createObjectURL(blob);
                    const link = document.createElement('a');
                    link.href = url;
                    link.download = `KEA_${hubModal.hub}_Terminals_${new Date().toISOString().slice(0, 10)}.csv`;
                    link.click();
                  }}
            ...
            <div className="px-6 py-3 border-b border-slate-200 bg-white flex items-center justify-between gap-4">
              <div className="relative flex-1 max-w-sm">
                ...
                <input
                  type="text"
                  placeholder="Search store, machine code, staff..."
                  value={hubModalSearch}
                  onChange={(e) => setHubModalSearch(e.target.value)}
              ...
              <div className="text-xs text-slate-500 font-mono">
                Showing <strong>{terminals.filter(t => t.hub === hubModal.hub).length}</strong> machines • <strong>{hubModal.merchandiserCount}</strong> staff
              </div>
            </div>
              ...
              <table className="w-full text-left text-xs text-slate-700">
                <thead className="bg-slate-50 text-slate-600 font-mono text-[10px] uppercase tracking-wider border-b border-slate-200 sticky top-0">
                  <tr>
                    ...
                    <th className="px-4 py-3">Commercial Zone</th>
                    <th className="px-4 py-3">Assigned Staff</th>
                    <th className="px-4 py-3">Status</th>
                    <th className="px-4 py-3">Battery &amp; Stock</th>
                ...
                <tbody className="divide-y divide-slate-100 font-mono">
                  {terminals
                    .filter(t => t.hub === hubModal.hub)
                    .filter(t => {
                      if (!hubModalSearch.trim()) return true;
                      const q = hubModalSearch.toLowerCase();
                      return (
                        t.terminalCode.toLowerCase().includes(q) ||
                        t.storeName.toLowerCase().includes(q) ||
                        t.zone.toLowerCase().includes(q) ||
                    ...
                    .map((item) => (
                      <tr key={item.id} className="hover:bg-slate-50/80 transition-colors">
                        <td className="px-4 py-3 font-bold text-slate-900">{item.terminalCode}</td>
                        <td className="px-4 py-3 font-semibold text-slate-800">{item.storeName}</td>
                        <td className="px-4 py-3 text-slate-500">{item.zone}</td>
                        <td className="px-4 py-3 font-bold text-slate-900">{item.assignedRep}</td>
                        <td className="px-4 py-3">
                          <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-bold ${
                            item.status === 'online' ? 'bg-emerald-50 text-emerald-700 border border-emerald-200' : 'bg-amber-50 text-amber-700 border border-amber-200'
                          }`}>
                            <CheckCircle2 className="w-3 h-3" /> {item.status.toUpperCase()}
                          </span>
                        </td>
                        <td className="px-4 py-3 text-slate-600">
                          <div>Bat: {item.battery}% • Stock: {item.stockLevel}%</div>
                          <div className="text-[10px] text-slate-400">{item.hourlyTransactions} tx/hr</div>
                        </td>
            ...
            <div className="px-6 py-3.5 border-t border-slate-200 bg-slate-50 flex items-center justify-between text-xs">
              <span className="text-slate-500 font-mono">Branch Hub Reconciliation: <strong>{hubModal.reconciliationRate}%</strong></span>
              <button
                onClick={() => {
                  setHubModal(null);
                  setHubModalSearch('');
